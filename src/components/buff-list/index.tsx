...
const BuffRenderer: FunctionalComponent<BuffRendererProps> = (props) => {
	...
	function getBuffEffectTypeText (type: BUFFEFFECT_TYPE, target: BUFF_ATTR_TYPE): preact.VNode {
		const tUP = <Locale k="BUFFTYPE_UP" />;
		const tDOWN = <Locale k="BUFFTYPE_DOWN" />;
		...
		switch (type) {
			...
			case BUFFEFFECT_TYPE.STAGE_REFLECTPHYSICS_RATIO_DEFENDER: // 29
				return <Locale k="BUFFTYPE_COUNT" />; // 피격자가 물리 반격
			...
			case BUFFEFFECT_TYPE.STAGE_CRITICAL_NEXTATTACK: // 54
				return <Locale k="BUFFTYPE_CRIT_NEXT" p={ [p] } />;
			case BUFFEFFECT_TYPE.STAT_RANGE_VALUE: // 55
				return <Locale k="BUFFTYPE_RANGE" p={ [p] } />;
			...
			case BUFFEFFECT_TYPE.STAGE_DEFPIERCE_RATIO: // 58
				return <Locale k="DEF_PIERCE" p={ [p] } />;
			...
			case BUFFEFFECT_TYPE.STAGE_TROOPERTYPEDMGBONUS_RATIO: // 60
				return <Locale k="BUFFTYPE_ANTI_LIGHT_DMG" p={ [p] } />;
			case BUFFEFFECT_TYPE.STAGE_ARMOREDTYPEDMGBONUS_RATIO: // 61
				return <Locale k="BUFFTYPE_ANTI_HEAVY_DMG" p={ [p] } />;
			case BUFFEFFECT_TYPE.STAGE_MOBILITYTYPEDMGBONUS_RATIO: // 62
				return <Locale k="BUFFTYPE_ANTI_MOBILITY_DMG" p={ [p] } />;
			...
			case BUFFEFFECT_TYPE.STAGE_PHYSICS_DAMAGE_APPLY: // 70
				return <Locale k="BUFFTYPE_DMG_PHYSICS_BY_HP" />;
			case BUFFEFFECT_TYPE.STAGE_FIRE_DAMAGE_APPLY: // 71
				return <Locale k="BUFFTYPE_DMG_FIRE_BY_HP" />;
			case BUFFEFFECT_TYPE.STAGE_ICE_DAMAGE_APPLY: // 72
				return <Locale k="BUFFTYPE_DMG_ICE_BY_HP" />;
			case BUFFEFFECT_TYPE.STAGE_LIGHTNING_DAMAGE_APPLY: // 73
				return <Locale k="BUFFTYPE_DMG_THUNDER_BY_HP" />;
			...
			case BUFFEFFECT_TYPE.STAGE_DAMAGEAMP_BYHP_ME: // 80
				return <Locale k="BUFFTYPE_DMG_UP_BY_HP" p={ [p] } />;
			case BUFFEFFECT_TYPE.STAGE_DAMAGEAMP_BYHP_OPP: // 81
				return <Locale k="BUFFTYPE_DMG_UP_BY_TARGET_HP" p={ [p] } />;
			...
			case BUFFEFFECT_TYPE.STAGE_DEBUFF_PERDOWN: // 91
				return <Locale k="BUFFTYPE_RES_DEBUFF" p={ [p] } />;
			case BUFFEFFECT_TYPE.STAGE_BUFFEFFECTRATE_CHANGE: // 92
				return <Locale k="BUFFTYPE_EFFECT_RATE" p={ [p] } />;
			...
			case BUFFEFFECT_TYPE.STAGE_EXP_UP: // 95
				return <Locale k="BUFFTYPE_EXP" p={ [p] } />;
			...
			case BUFFEFFECT_TYPE.STAGE_IMMUNITY_DEBUFF: // 101
				return <Locale k="BUFFTYPE_DEBUFF_IMMUNE" p={ [p] } />; // 효과 면역
	...
	function getTriggerText (trigger: BuffTrigger): preact.VNode {
		if (typeof trigger === "string") {
			switch (trigger) {
				...
				case "enemy_killed":
					return <Locale k="BUFFTRIGGER_KILL" />;
				case "criticaled":
			...
			if ("_comment" in trigger)
				...
				if (typeof trigger["hp>="] === "string")
					return <Locale k="BUFFTRIGGER_HP_>=" p={ ["", trigger["hp>="]] } />;

				const target = {
					self: <Locale k="BUFFTARGET_SELF" />,
					target: <Locale k="BUFFTARGET_TARGET" />,
				}[trigger["hp>="].target];
				return <Locale k="BUFFTRIGGER_HP_>=" p={ [target, trigger["hp>="].value] } />;
			} else if ("hp<=" in trigger) {
				if (typeof trigger["hp<="] === "string")
					return <Locale k="BUFFTRIGGER_HP_<=" p={ ["", trigger["hp<="]] } />;
				...
				if (typeof trigger.in_squad === "string")
					return <Locale k="BUFFTRIGGER_IN_SQUAD" p={ [convertBuff(trigger.in_squad)] } />;

				const src = trigger.in_squad
					.map(convertBuff)
					.unique(VNodeUnique);
				return <Locale k="BUFFTRIGGER_IN_SQUAD" p={ [
					<Fragment>{ src.gap(<Locale k="BUFFTRIGGER_OR" />) }</Fragment>,
				] } />;
			} else if ("in_enemy" in trigger) {
				if (typeof trigger.in_enemy === "string")
					return <Locale k="BUFFTRIGGER_IN_ENEMY" p={ [convertBuff(trigger.in_enemy)] } />;
				...
				if ("func" in trigger.on && Array.isArray(trigger.on.select)) {
					...
					return <Locale k={ `BUFFTRIGGER_ON_STACK_TARGET_${select.length === 1 ? "SINGLE" : "MULTIPLE"}` } p={ params } />;
				...
				if (trigger.target.length === 1)
					return <Locale k="BUFFTRIGGER_ON_TARGET_SINGLE_OR" p={ [convertBuff(trigger.target[0])] } />;
	...
	function getBuffText (stat: BuffEffect, level?: number): preact.VNode {
		if ("_comment" in stat)
			...
			if (typeof stat.off === "string")
				return <Locale k="BUFFEFFECT_OFF" p={ [stat.off] } />;
			else if (typeof stat.off === "number") {
				const target = {
					0: <Locale k="BUFFEFFECT_OFF_TYPE_0" />,
					1: <Locale k="BUFFEFFECT_OFF_TYPE_1" />,
					2: <Locale k="BUFFEFFECT_OFF_TYPE_2" />,
					3: <Locale k="BUFFEFFECT_OFF_TYPE_3" />,
					4: <Locale k="BUFFEFFECT_OFF_TYPE_4" />,
				}[stat.off];
				return <Locale k="BUFFEFFECT_OFF" p={ [target] } />;
			...
			switch (stat.resist.type) {
				case "debuff":
					return <Locale k="BUFFEFFECT_RES_DEBUFF" p={ [signedValue(stat.resist.value, level)] } />;
			}
			...
			if ("elem" in stat.fixed_damage) {
				switch (stat.fixed_damage.elem) {
					case "fire":
						return <Locale k={ isRatioValue(stat.fixed_damage.damage) ? "BUFFEFFECT_DOT_FIRE_RATIO" : "BUFFEFFECT_DOT_FIRE" } p={ [
							nsignedValue(stat.fixed_damage.damage, level),
							<ElemIcon elem={ stat.fixed_damage.elem } class="me-1 mb-0" />,
						] } />;
					case "ice":
						return <Locale k={ isRatioValue(stat.fixed_damage.damage) ? "BUFFEFFECT_DOT_ICE_RATIO" : "BUFFEFFECT_DOT_ICE" } p={ [
							nsignedValue(stat.fixed_damage.damage, level),
							<ElemIcon elem={ stat.fixed_damage.elem } class="me-1 mb-0" />,
						] } />;
					case "lightning":
						return <Locale k={ isRatioValue(stat.fixed_damage.damage) ? "BUFFEFFECT_DOT_THUNDER_RATIO" : "BUFFEFFECT_DOT_THUNDER" } p={ [
							nsignedValue(stat.fixed_damage.damage, level),
							<ElemIcon elem={ stat.fixed_damage.elem } class="me-1 mb-0" />,
						] } />;
				}
			}
			return <Locale k={ isRatioValue(stat.fixed_damage) ? "BUFFEFFECT_DOT_PHYSICS_RATIO" : "BUFFEFFECT_DOT_PHYSICS" } p={ [
			...
			return <Locale k="BUFFEFFECT_TOGETHER" p={ [
