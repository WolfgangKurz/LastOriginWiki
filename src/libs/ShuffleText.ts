...
export default class ShuffleText {
	...
	/** *MODIFIED* \
	 * waiting time to delay between frames
	 */
	public timeGap: number = 0;
	...
	private _onInterval (): void {
		// MODIFIED, use timeGap
		const now = new Date().getTime();
		const timeElapsed = now - this._timeLast;
		if (timeElapsed < this.timeGap) {
			if (this._isRunning) {
				this._requestAnimationFrameId = requestAnimationFrame(() => {
					this._onInterval();
				});
		...
		this._timeLast = now;
		this._timeCurrent = this._timeLast - this._timeStart;

		// const percent = this._timeCurrent / this.duration;
		// MODIFIED, use easeOutCubic
		const percent = ((x) => 1 - Math.pow(1 - x, 3))(this._timeCurrent / this.duration);
