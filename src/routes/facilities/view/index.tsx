...
const FacilityView: FunctionalComponent<FacilityViewProps> = (props) => {
	...
	function TimeFormat (duration: number): preact.VNode {
		...
		const dt = [th, tm, ts].map(r => r.toString().padStart(2, "0")).join(":");
	...
	SetMeta(
		...
		`${loc[`FACILITY_${facility.uid}`]} 설비의 정보입니다. 소모 자원과 필요 전투원 정보, 생산품과 업그레이드 비용/시간을 확인할 수 있습니다.`,
	);
	...
	const entry = facility.list[level - 1];
					...
					<div class="row text-center">
						...
						<div class="col-6 col-md-9">
							<input
								...
								max="25"
								value={ level }
								onInput={ (e): void => setLevel(parseInt((e.target as HTMLInputElement).value, 10)) }
							/>
						</div>
					</div>
