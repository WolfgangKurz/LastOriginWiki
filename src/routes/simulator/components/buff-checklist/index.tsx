			...
			{ ((): preact.VNode => {
				switch (type) {
					...
					case BUFFEFFECT_TYPE.STAGE_DEBUFF_PERDOWN: // 91
						return <Locale k="BUFFTYPE_RES_DEBUFF" />;
					...
					case BUFFEFFECT_TYPE.EVADE_SKILLDMGDOWN: // 112
						return <Locale k="BUFFTYPE_BY" p={ [
							<Locale k="BUFFTARGET_BY_ATTACKER" />,
							<Locale k="BUFFEFFECT_BY_EVADE" />,
							<Locale k="BUFFEFFECT_BY_SKILL_RATIO" />,
						] } />;
		...
		if (typeof trigger === "string") {
			switch (trigger) {
				...
				case "enemy_killed_passive":
					return <Locale k="BUFFTRIGGER_KILL_PASSIVE" />;
				case "enemy_killed_counter":
					return <Locale k="BUFFTRIGGER_KILL_COUNTER" />;
				case "criticaled":
			...
			if ("_comment" in trigger)
				...
				switch (trigger.damaged) {
					...
					case "skill":
						return <Locale k="BUFFTRIGGER_DAMAGED_SKILL" p={ [convertBuff(trigger.key)] } />;
				}
	...
	function getBuffText (stat: BuffEffect, level?: number): preact.VNode {
		if ("_comment" in stat)
			...
			if (typeof stat.off === "string") {
				...
				const target = {
					0: <Locale k="BUFFEFFECT_ATTR_0" />,
					1: <Locale k="BUFFEFFECT_ATTR_1" />,
					2: <Locale k="BUFFEFFECT_ATTR_2" />,
					3: <Locale k="BUFFEFFECT_ATTR_3" />,
					4: <Locale k="BUFFEFFECT_ATTR_4" />,
					5: <Locale k="BUFFEFFECT_ATTR_5" />,
					6: <Locale k="BUFFEFFECT_ATTR_6" />,
				}[stat.off];
