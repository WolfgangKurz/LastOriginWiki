...
import { SkillSummonInfo } from "@/types/DB/Skill";
...
const SummonBadge: FunctionalComponent<SummonBadgeProps> = (props) => {
	...
	return <div class={ `summon-badge ${props.class || ""}` }>
		<span class="badge bg-dark summon-badge" role="button" onClick={ (): void => setDisplay(true) }>
		...
		>
			...
			<div class="container mb-3">
				<div class="row row-cols-6 summon-display-tabs mt-1">
					{ Skills.map((skill, idx) => <div
						class={ [
							"col",
							"mt-1",
							displayTab === `skill${idx + 1}` ? "border border-bottom-0" : "border-bottom",
						].join(" ") }
						role="button"
						onClick={ (e): void => {
							e.preventDefault();
							setDisplayTab(`skill${idx + 1}`);
						} }
					>
						{ skill
							? <img
					...
					<div
						class={ `col mt-1 ${displayTab === "ai" ? "border border-bottom-0" : "border-bottom"}` }
						role="button"
						onClick={ (e): void => {
							e.preventDefault();
							setDisplayTab("ai");
				...
				{ Skills.map((skill, idx) => displayTab === `skill${idx + 1}` && skill
					? <div class="row">
						<div class="col border border-top-0 text-start p-3">
							...
							<div class="skill-desc">
								<div>
									...
									{ skill.target_ground
										? <span class="badge bg-danger me-1" title={ loc["ENEMY_SKILL_GRID_TARGET_TIP"] }>
