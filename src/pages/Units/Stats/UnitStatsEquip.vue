...
export default class UnitStatsEquip extends Vue {
	private DB: LazyDataType<Equip[]> = null;
	private InitialDB () {
		this.DB = null;

		LazyLoad(
			r => {
				const Equip = r[0] as Equip[];
	...
	private EquipName (key: string) {
		const eq = this.DB && this.DB.find(x => x.fullKey === key);
		if (eq) return eq.name;
		return key;
	}

	private FindEquip (key: string) {
		return (this.DB && this.DB.find(x => x.fullKey === key)) || Equip.Empty;
	}
