...
export class CanvasRenderer extends Renderer {
	...
	renderReplacedElement (
		...
		if (image && container.intrinsicWidth > 0 && container.intrinsicHeight > 0) {
			...
			if (box.width / container.intrinsicWidth < 0.5 || box.height / container.intrinsicHeight < 0.5) {
				let _w = container.intrinsicWidth;
				let _h = container.intrinsicHeight;
				...
				while (_w > box.width) {
					const cw = Math.max(box.width, _w * 0.5);
					const ch = Math.max(box.height, _h * 0.5);

					console.log(_w, cw);
					_ctx.drawImage(_cv, 0, 0, _w, _h, 0, 0, cw, ch);
				...
				_cv2.width = box.width;
				_cv2.height = box.height;

				const _ctx2 = _cv2.getContext("2d")!;
				_ctx2.drawImage(_cv, 0, 0);
