	...
	targetDBs.forEach(targetDB => {
		...
		sheets.spreadsheets.values.get({
			...
			if (rows && rows.length) {
				...
				fs.mkdirSync(path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike"), { recursive: true });
				fs.writeFileSync(
					path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike", "quest.json"),
					JSON.stringify(ret),
				);
			} else
		...
		sheets.spreadsheets.values.get({
			...
			if (rows && rows.length) {
				...
				fs.mkdirSync(path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike"), { recursive: true });
				fs.writeFileSync(
					path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike", "node.json"),
					JSON.stringify(ret),
				);
			} else
				console.log("No data found.");
		...
		sheets.spreadsheets.values.get({
			...
			if (rows && rows.length) {
				...
				fs.mkdirSync(path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike"), { recursive: true });
				fs.writeFileSync(
					path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike", "limiteffects.json"),
					JSON.stringify(ret),
				);
			} else
				console.log("No data found.");
		...
		sheets.spreadsheets.values.get({
			...
			if (rows && rows.length) {
				...
				fs.mkdirSync(path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike"), { recursive: true });
				fs.writeFileSync(
					path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike", "item.json"),
					JSON.stringify(ret),
				);
			} else
				console.log("No data found.");
		...
		sheets.spreadsheets.values.get({
			...
			if (rows && rows.length) {
				...
				fs.mkdirSync(path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike"), { recursive: true });
				fs.writeFileSync(
					path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike", "buffeffects.json"),
					JSON.stringify(ret),
				);
			} else
				console.log("No data found.");
		...
		sheets.spreadsheets.values.get({
			...
			if (rows && rows.length) {
				...
				fs.mkdirSync(path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike"), { recursive: true });
				fs.writeFileSync(
					path.resolve(__dirname, "..", "..", "external", "json", targetDB, "roguelike", "effects.json"),
					JSON.stringify(ret),
				);
			} else
				console.log("No data found.");
